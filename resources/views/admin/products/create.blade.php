...
<script>
    ...
    $("#productForm").submit(function (event) {
        ...
        $("button[type='submit']").prop('disabled', true);
        $.ajax({
            url: "{{ route('product.store') }}", // Your form submission route
            type: 'post',
            data: formArray,
            dataType: 'json',
            success: function (response) {
    ...
    const dropzone = new Dropzone("#image", {
        ...
        success: function (file, response) {
            ...
            if (response.image_id && response.imagePath) {
                $("#image_id").val(response.image_id);

                // HTML template for the uploaded image card
                let html = `
    <div class="col-md-3" id="image-row-${response.image_id}">
        <div class="card">
            <input type="hidden" name="image_array[]" value="${response.image_id}">
            <img src="${response.imagePath}" class="card-img-top" alt="Product Image"
                onerror="this.onerror=null; this.src='/path/to/default-image.jpg'; console.error('Image not found:', this.src);">
        ...
        error: function (file, errorMessage) {
            ...
            this.removeFile(file); // Remove the file from Dropzone on error
        },
        complete: function (file) {
            this.removeFile(file); // Remove file preview after upload completes
        }
    });
